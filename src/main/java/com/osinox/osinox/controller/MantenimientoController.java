...
import com.osinox.osinox.service.MantenimientoService;
...
@PreAuthorize("hasAnyRole('ADMIN','GERENTE','JEFE_TALLER','TECNICO')")
public class MantenimientoController {
 
    private final MantenimientoService mantenimientoService;
 
    @GetMapping
    public ResponseEntity<Page<MantenimientoDto>> getAll(
            @PageableDefault(size = 15) Pageable pageable) {
        Page<MantenimientoDto> page = mantenimientoService.getAll(pageable);
    ...
    public ResponseEntity<?> create(@RequestBody MantenimientoDto dto) {
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(mantenimientoService.create(dto));
        } catch (RuntimeException e) {
    ...
    public ResponseEntity<?> update(@PathVariable Long id, @RequestBody MantenimientoDto dto) {
        try {
            return ResponseEntity.ok(mantenimientoService.update(id, dto));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> cambiarEstado(@PathVariable Long id, @RequestParam String estado) {
        try {
            return ResponseEntity.ok(mantenimientoService.cambiarEstado(id, estado));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        }
    }
